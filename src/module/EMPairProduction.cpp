	...
	public:
		...
		std::vector<double> getDistribution(double s)
		{
			if (s < s_values.front() || s > s_values.back())
				std::cerr << "Warning tabulated s range not sufficient PP " << s / eV / eV << std::endl;
...
void EMPairProduction::performInteraction(Candidate *candidate) const {
	...
	if (haveElectrons){
		...
		if (tabs[i+j] > 4*mec2*mec2 && tabs[i+j-1] < 4*mec2*mec2){ // cdf = 0 for s < 4*me**2, but bin edge not at s = 4*me**2. So bin ranges from 3.91*me**2 to 4.16*me**2 (resp. 3.93*me**2 to 4.06*me**2) which can lead to s values below physical boundary of process by chosing s uniformly in binsize. Therefore bin size is restricted to physical range.
			binWidth = (tabs[i+j] - 4.*mec2*mec2);
			s_kin = 4*mec2*mec2 + random.rand()*binWidth;
		}
		s_kin *= (1 + z);
		if (s_kin < 4*mec2*mec2){
			std::cout << "ERROR PP " << tabs[i+j-1] / eV / eV << std::endl;
			return;
		}
