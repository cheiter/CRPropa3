...
class ICSSecondariesEnergyDistribution
{
	...
	public:
		...
		{
			if (s_min < 1.01* mass_electron*c_squared * mass_electron*c_squared)
			{
				std::cerr << "Warning: Minimum COM Energy in ICS Interpolation s = " << s_min << " < 1.01 * m_e**2 selected. Setting to s_min = m_e**2.\n" ;
				s_min = 1.01 *mass_electron*c_squared * mass_electron*c_squared;
			}
			_Ns = Ns;
			_Nrer = Nrer;
			_s_min =s_min;
